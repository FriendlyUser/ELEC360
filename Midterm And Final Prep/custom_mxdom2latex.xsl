...
<!-- 
The consequent commands are chained up into one mcodeoutput node, 
thats why one could get unwanted "</latex> <latex>" in the middle of the mcodeoutput after two consequent disp(['<latex></latex>']) injections
...
\includegraphics [width=7in]{<xsl:value-of select="@src"/>}
</xsl:template>
